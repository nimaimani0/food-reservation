...
void edit_food()
{
    ...
    cin >> foodID;
    Food foods;
    ofstream efile("edittt.txt");

    bool fo = false;

    while (file >> foods.foodname >> foods.price >> foods.day >> foods.dinnerorlunch >> foods.date >> foods.capacity >> foods.id)
    {
        if (foods.foodname == editfood && foods.id == foodID)
        {
            fo = true;
            cout << "price :  " << foods.price << endl;
            cout << "new food price : ";
            cin >> foods.price;
            cout << "capacity : " << foods.capacity << endl;
            cout << "new food capacity : ";
            cin >> foods.capacity;
        }
        efile << foods.foodname << " " << foods.price << " "
              << foods.day << " " << foods.dinnerorlunch << " "
              << foods.date << " " << foods.capacity << " " << foods.id << endl;
    }
